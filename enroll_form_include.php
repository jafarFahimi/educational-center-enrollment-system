...
$teacher = $conn->query("SELECT *,concat(firstname,' ',middlename,' ',lastname) as name FROM teacher where status = 1");
$fac_arr = array();
while($row=$teacher->fetch_assoc()){
	$fac_arr[$row['class_section_id']] = ucwords($row['name']);
}
if(isset($_POST['id'])){
	$qry = $conn->query("SELECT e.*,s.firstname,s.lastname,s.middlename,s.gender,s.dob,s.address,l_s.* from enrollment e inner join teacher f on f. id = e.teacher_id inner join class_section ls on e.class_section_id = ls.id inner join student_list s on s.id = e.student_id inner join last_school l_s on l_s.enrollment_id = e.id where e.id=".$_POST['id']);
...
?>
		...
		<?php endif; ?>
			...
			<div class="form-group">
			<label for="class_section_id">Enroll to </label>
			<select name="class_section_id" id="class_section_id" class="custom-select browser-default select2">
				<option ></option>
				<?php  
				$ls = $conn->query("SELECT *,concat(class,'-',section) as ls from class_section where status = 1 ");
				while($row=$ls->fetch_assoc()){
					if(isset($fac_arr[$row['id']])){
				?>
				<option value="<?php echo $row['id'] ?>" <?php echo isset($meta['class_section_id']) && $meta['class_section_id'] == $row['id'] ? 'selected' : '' ?>><?php echo (ucwords($row['ls'])) ."  (Adviser: " . $fac_arr[$row['id']] . " )" ?></option>
				<?php }} ?>	
			</select>
