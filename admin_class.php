...
Class Action {
    ...
    $this->db = $conn;
	/*
	 "$this->db = $conn;" is assigning the value of the variable $conn to the property $db of the current object. This means that the $db property will hold the value of the database connection.
	*/
	}

	function __destruct() {
	    $this->db->close();
		// "$this->db->close();" is calling the close() method on the $db property. This is used to close the database connection.
// The "->" operator is used to access properties and methods of an object. 
	...
	function save_class_section(){
		extract($_POST);
		if(empty($id)){
			$chk = $this->db->query("SELECT * from class_section where class ='".$class."' and section = '".$section."' ")->num_rows;
			if($chk > 0){
				return json_encode(array('status'=>2,'msg'=>'class and Section already exist'));
			}else{
				$save = $this->db->query("INSERT INTO class_section set class ='".$class."' , section = '".$section."' ");
				if($save)
					return json_encode(array('status'=>1));
			}
		}else{
			$chk = $this->db->query("SELECT * from class_section where class ='".$class."' and section = '".$section."' and id !='".$id."' ")->num_rows;
			if($chk > 0){
				return json_encode(array('status'=>2,'msg'=>'class and Section already exist'));
			}else{
				$save = $this->db->query("UPDATE class_section set class ='".$class."' , section = '".$section."' where id=".$id);
				if($save)
					return json_encode(array('status'=>1));
			}
		}
	...
	function save_teacher(){
		extract($_POST);
		if(empty($id)){
			$chk = $this->db->query("SELECT * from teacher where firstname ='".$firstname."' and lastname = '".$lastname."' and middlename = '".$middlename."' ")->num_rows;
			if($chk > 0){
				return json_encode(array('status'=>2,'msg'=>'teacher already exist'));
			}else{
				...
				return json_encode(array('status'=>2,'msg'=>'Username already exist'));
					...
					if($save){
						...
						$save2 = $this->db->query("INSERT INTO teacher set firstname ='".$firstname."' , lastname = '".$lastname."' , middlename = '".$middlename."' ,user_id = '".$uid."' , class_section_id = '".$class_section_id."'  ");
						if($save2)
						return json_encode(array('status'=>1));
					}
				}
			...
			$chk = $this->db->query("SELECT * from teacher where firstname ='".$firstname."' and lastname = '".$lastname."' and middlename = '".$middlename."' and id != '".$id."' ")->num_rows;
			if($chk > 0){
				return json_encode(array('status'=>2,'msg'=>'teacher already exist'));
			}else{
				...
				return json_encode(array('status'=>2,'msg'=>'Username already exist'));
					...
					if($save){
						$save2 = $this->db->query("UPDATE teacher set firstname ='".$firstname."' , lastname = '".$lastname."' , middlename = '".$middlename."' ,user_id = '".$uid."' , class_section_id = '".$class_section_id."' where id = '".$id."'  ");
						if($save2)
						return json_encode(array('status'=>1));
					}
				}
	...
	function save_enroll(){
		...
		if($type == 1 || $type == 3){
			...
			$fid = $this->db->query("SELECT * FROM teacher where class_section_id = '".$class_section_id."' and status = 1 ");
	...
	function load_class_section(){
		$qry = $this->db->query("SELECT * from class_section where status = 1");
		$data = array();
	...
	function load_student(){
		$qry = $this->db->query("SELECT e.*,CONCAT(s.firstname,' ',s.middlename,' ',s.lastname) as name,CONCAT(f.firstname,' ',f.middlename,' ',f.lastname) as fname,CONCAT(ls.class,'-',ls.section) as ls,s.type from enrollment e inner join teacher f on f. id = e.teacher_id inner join class_section ls on e.class_section_id = ls.id inner join student_list s on s.id = e.student_id  where e.status = 1 and e.school_year in (SELECT id from school_year where is_on = 1 ) ");
		$data = array();
		while($row=$qry->fetch_assoc()){
			if($row['type'] == 1)
				$row['user_type'] = "New";
	...
	function load_teacher(){
		$qry = $this->db->query("SELECT f.*,concat(l.class,'-',l.section) as class from teacher f inner join class_section l on f.class_section_id = l.id where f.status = 1");
		$data = array();
		while($row=$qry->fetch_assoc()){
			$data[] = $row;
		}
	...
	function remove_class_section(){
		extract($_POST);
		$remove = $this->db->query("UPDATE class_section set status = 0 where id =".$id);
		if($remove)
			return 1;
	}
	function remove_teacher(){
		extract($_POST);
		$remove = $this->db->query("UPDATE teacher set status = 0 where id =".$id);
		if($remove)
			return 1;
	}
