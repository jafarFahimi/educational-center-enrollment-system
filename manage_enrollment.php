...
if(isset($_GET['id'])){
	$qry = $conn->query("SELECT e.*,CONCAT(s.firstname,' ',s.middlename,' ',s.lastname) as name,CONCAT(f.firstname,' ',f.middlename,' ',f.lastname) as fname,CONCAT(ls.class,'-',ls.section) as ls,s.type from enrollment e inner join teacher f on f. id = e.teacher_id inner join class_section ls on e.class_section_id = ls.id inner join student_list s on s.id = e.student_id where e.id=".$_GET['id']);
